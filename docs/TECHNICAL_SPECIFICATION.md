...
The project has been restructured into separate components following professional software development practices:
...
See [database/README.md](../database/README.md) and [database/database_schema.sql](../database/database_schema.sql) for detailed schema information.

## System Components

### 1. Mobile Application
The Android application is the primary interface for students to mark their attendance. Key features include:

- Bluetooth beacon scanning for proximity verification
- GPS and WiFi data collection for location validation
- QR code scanning for session verification
- Secure authentication with faculty-issued credentials

### 2. Web Portal
The web portal provides interfaces for different user roles:

- **Admin Dashboard**: System configuration, user management, and analytics
- **Faculty Portal**: Class management, attendance session initiation, and reporting
- **Student Portal**: Attendance history and personal information
- **SmartBoard Display**: Real-time attendance visualization for classrooms

### 3. Backend Services
The backend provides RESTful APIs and business logic:

- User authentication and authorization
- Attendance session management
- QR code generation and validation
- Data processing and analytics
- Bluetooth proximity coordination

### 4. Database
The database stores all system information:

- User accounts (students, faculty, administrators)
- Class schedules and enrollments
- Attendance records and session data
- Bluetooth beacon registrations
- QR code tokens and validation information

## Communication Flow

1. Faculty initiates an attendance session via the web portal
2. System generates a dynamic QR code with a 5-second refresh interval
3. Students scan the QR code using the mobile app
4. Mobile app performs Bluetooth beacon scanning for proximity verification
5. GPS and WiFi data are collected for location validation
6. Attendance data is sent to the backend for processing
7. Results are displayed in real-time on the SmartBoard
8. Reports and analytics are available through the web portal

## Security Measures

- Password hashing with bcrypt
- JWT token-based authentication
- QR code token rotation every 5 seconds
...
- Mobile app deployed through Google Play Store
- Web portal hosted on web server
- Backend API deployed on application server
- Database hosted on dedicated database server
- QR codes generated dynamically and served via API
